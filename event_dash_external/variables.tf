...
variable "rds_force_ssl" {
  description = "Require SSL for RDS connections (rds.force_ssl=1). Set to false so FastSchema/pgx can connect without SSL until client supports it."
  type        = bool
  default     = false
}

variable "force_destroy" {
  description = "Allow destructive delete: S3 bucket can be destroyed with contents; RDS skips final snapshot on delete. Use with care."
  type        = bool
  default     = false
}
